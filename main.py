...
classifiers = {
    ...
    FOREST: lambda d: ensemble.RandomForestClassifier(max_depth=d)
}
...
meta = {
    ...
    FOREST: {
        ...
        XLABEL: 'max depth',
        FILENAME: 'random_forrest_res.txt',
        PHOTONAME: 'random_forrest_graph.txt',
        RANGE: [i for i in range(1, 101)]
    }
}
...
def optimize(data, values, kernel):
    ...
    def _optimize(index):
        for i in subsets[index]:
            ...
            e = (utils.kfold(data, i, kernel, k=5), i)
            lock.acquire()
            q.put(e)
            lock.release()
